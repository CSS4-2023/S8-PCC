...
LOG_FILE="/vagrant/logs/install_nrpe_web.log"
APT_OPT="-o Dpkg::Progress-Fancy="0" -q -y"

echo "START - Install NRPE on Vault Server - "$IP
...
echo "=> [5]: NRPE Commands Configuration"
...
command[check_mem]=/usr/local/nagios/libexec/check_mem.pl -f -w 20 -c 10" >> /usr/local/nagios/etc/nrpe.cfg

systemctl restart nrpe.service >> $LOG_FILE 2>&1
systemctl status nrpe.service >> $LOG_FILE 2>&1

echo "END - Install NRPE on Vault Server"