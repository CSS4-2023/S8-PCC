...
if [ -n "$DBNAME" ] && [ -n "$DBUSER" ];then
  ...
  sudo mysql -e "CREATE USER '$DBUSER'@'Web1.e4.sirt.tp' identified by '$DBPASSWD'"
  ...
  sudo mysql -e "CREATE USER '$DBUSER'@'Web2.e4.sirt.tp' identified by '$DBPASSWD'"
  sudo mysql -e "grant all privileges on $DBNAME.* to '$DBUSER'@'Web1.e4.sirt.tp'"
  sudo mysql -e "grant all privileges on $DBNAME.* to '$DBUSER'@'localhost'"
  sudo mysql -e "grant all privileges on $DBNAME.* to '$DBUSER'@'Web2.e4.sirt.tp'" 
...
pathmysqldump=$(ssh -o "StrictHostKeyChecking=no" -i /vagrant/data/backup_key root@192.168.4.110 'ls -t /root/bup_db_web/bup_hour_db_web/* | head -1')
